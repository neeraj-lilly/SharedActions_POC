...
open class BarLineScatterCandleBubbleRenderer: DataRenderer
{
    ...
    //  [rashmi - May 17]
    /// Modified the file and added a fucntion to return the bool value for showing Circular Dots
    /// This is done to achieve, drawing of Circular Dots on top of each graph point

    /// - returns: `true` if the Circular Dots Should be drawn, `false` if not.
    internal func shouldShowCircularDots(forDataSet set: IChartDataSet) -> Bool
    {
        return set.isShowCircularDotsEnabled
    }
