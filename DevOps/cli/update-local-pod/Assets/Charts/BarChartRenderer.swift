...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    {
        drawCircularDots(context: context)
    }

    //  MARK: Circular  Customization
    fileprivate func drawCircularDots(context: CGContext)
    {
        guard
            let dataProvider = dataProvider,
            let barData = dataProvider.barData
        ...
        for dataSetIndex in 0 ..< barData.dataSetCount
        {
            ...
            for i in stride(from: 0, to: min(Int(ceil(Double(dataSet.entryCount) * animator.phaseX)), dataSet.entryCount), by: 1)
            {
                ...
                //  MARK: Circular Dots Customization
                //  [rashmi - May 17]
                /// Check if the bool value for showing Circular Dots is true
                /// If true- get the x and y point to draw Circular Dots
                /// the data attribute of graph data object should have and UIimage
                /// check if image is present and draw the image at given x, y position in the context
                if shouldShowCircularDots(forDataSet: dataSet) {
                    if let circularDotColorImage = e.data as? UIImage {
                        circularDotColorImage.draw(in: CGRect(x: barRect.origin.x-1.0, y: barRect.origin.y + barRect.size.height-(circularDotColorImage.size.width/2), width: circularDotColorImage.size.width, height: circularDotColorImage.size.height))
                    }
                }
            }
        }
